...
public class AltDPElection extends AltElection{
    ...
    protected AltDPElection(ElectionParameters params) {
        setElection(params);
    ...
    public ArrayList<AltElectionState> findNE() throws Exception{
        ...
        Object2ObjectOpenHashMap<IVector, List<DPInfo>> gMap =
                new Object2ObjectOpenHashMap<>(IntMath.binomial(numVoters + numBoxes - 2, numBoxes -1));//TODO: check if this is correct
    ...
    private ArrayList<AltElectionState> generateWinnerStates(List<DPInfo> dpInfos, int numAlternatives,
                                                          int stateSize) {
        Queue<Triple<AltElectionState, DPInfo, IVector>> q = new ArrayDeque<>();
        ArrayList<AltElectionState> res = new ArrayList<>();
        for (DPInfo item : dpInfos) {
            IVector key = generateZeroVector(stateSize);
            AltElectionState initState = new AltElectionState(numAlternatives, getParams().canAbstain(), getParams().getRule());
        ...
        while (!q.isEmpty()) {
            Triple<AltElectionState, DPInfo, IVector> tuple = q.remove();
            AltElectionState state = tuple.first;
            ...
            if (info.getE() != null) {
                ...
                AltElectionState newState = null;
    ...
    private AltElectionState prepNewState_revised(AltElectionState state, IVector e) {
    	ScoreVector vector = state.getCurrentScores();
    	ArrayList<IVector> votes = state.getCurrentVotes();
    	ArrayList<Integer> candArray = getParams().getRule().getWinnersOfVoteVector(e, getParams());
        int candidate;
        if (candArray.size() > 00) candidate = candArray.get(0); else candidate = 0;
        votes.add(e);
        if (candidate == 0) {
        	//abstention - so do not use compilation function
        	return new AltElectionState(vector, state.getCurrentWinners(), votes, getParams().canAbstain(), getParams().getRule());
        } else {
        	vector = (ScoreVector) getParams().getRule().compilationFunction(vector, e, getParams());
        	return new AltElectionState(vector, getWinnersOfScoreVector(vector), votes, getParams().canAbstain(), getParams().getRule());
        }
    }
